async function getRegion(region) {
  const api_url = "https://datasource.kapsarc.org/api/records/1.0/search/?dataset=saudi-arabia-coronavirus-disease-covid-19-situation&rows=70&sort=date&refine.daily_cumulative=Cumulative&refine.region=" + region;
  ...
  for (let i = 0; data.records[i].fields.date == date; i++) {
      switch (data.records[i].fields.indicator) {
          ...
          case "Recoveries":
              ...
              break;
          default:
              console.log("Unkown indicator:" + data.records[i].fields.indicator);
              break;
      }
  }
